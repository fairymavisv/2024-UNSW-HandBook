...
use crate::{utlis::{StudyLevel, Campus, OfferingTerm}, requirements::{Requirements}};
...
impl Course {
    ...
    pub fn is_eligable(&self, program_code: &str, taken_course:&Vec<String>, wam: &Option<u8>, course_manager: &CourseManager) -> Result<bool, String> {
        if self.requirements.is_none() {
            println!("Warning: Requirements parsing error was happen");
            Ok(true)
        } else {
            self.requirements.as_ref().unwrap().is_satisified(program_code, taken_course, wam, course_manager)
        }
    }
...
impl CourseManager {
    ...
    fn parse_from_courses_json(json_path: &str) -> HashMap<String, Course> {
        ...
        let course_list: HashMap<String, Course> = json_courses.courses
        // .into_iter()
        .into_par_iter()
...
mod tests {
    ...
    #[test]
    fn test_read_json() {
        let api = CourseManager::new("/home/shilong/UNSW-HandBookX/backend/data/coursesProcessed.json",
    ...
    fn test_get_course() {
        ...
        assert_eq!(course.offering_terms, vec![OfferingTerm::Term1, OfferingTerm::Term2, OfferingTerm::Term3])
    }
    ...
    fn test_get_eqv() {
        ...
        "/home/shilong/UNSW-HandBookX/backend/data/equivalents.json",
        "/home/shilong/UNSW-HandBookX/backend/data/exclusions.json");
        assert!(api.equivalent_courses.contains_key(&String::from("COMP1511")));
        let courses = api.equivalent_courses.get("COMP1511").unwrap();
        assert_eq!(courses, &vec![String::from("COMP1917"), String::from("DPST1091")]);
    ...
    fn test_get_excl() {
        ...
        "/home/shilong/UNSW-HandBookX/backend/data/equivalents.json",
        "/home/shilong/UNSW-HandBookX/backend/data/exclusions.json");
        assert!(api.exclusion_courses.contains_key(&String::from("COMP1511")));
    ...
    fn test_course_empty_requirement() {
        ...
        "/home/shilong/UNSW-HandBookX/backend/data/exclusions.json");
        let course = api.get_course(&CourseCode::from("COMP1511")).unwrap();
        assert!(course.is_eligable("3778", &vec![], &None, &api).unwrap());
    }
    ...
    fn test_course_requriements_course() {
        ...
        assert!(course.is_eligable("3778", &vec![String::from("MATH1081")], &None, &api).unwrap());
    }
    ...
    fn test_course_requriements_program() {
        ...
        assert!(!course.is_eligable("9999", &vec![], &None, &api).unwrap());
        assert!(course.is_eligable("4515", &vec![], &None, &api).unwrap());
        assert!(course.is_eligable("3648", &vec![], &None, &api).unwrap());
    }
    ...
    fn test_course_requriements_wam() {
        ...
        assert!(course.is_eligable("4522", &vec![], &Some(70), &api).unwrap());
        assert!(!course.is_eligable("4522", &vec![], &Some(55), &api).unwrap());
    ...
    fn test_course_requriements_uoc() {
        ...
        "/home/shilong/UNSW-HandBookX/backend/data/exclusions.json");
        let course = api.get_course(&CourseCode::from("BLDG4018")).unwrap();
        assert!(course.is_eligable("4522", &vec![String::from("DART1141")], &None, &api).unwrap());
    }
    ...
    fn test_course_requriements_binary() {
        ...
        assert!(course.is_eligable("3784", &vec![String::from("COMP1531"), String::from("COMP2521")], &None, &api).unwrap());
        assert!(course.is_eligable("3784", &vec![String::from("COMP1531"), String::from("COMP1927")], &None, &api).unwrap());
    ...
    fn test_course_requriements_list() {
        ...
        assert!(course.is_eligable("3784", &vec![String::from("MATH1081"), String::from("COMP2521")], &None, &api).unwrap());
        assert!(course.is_eligable("3784", &vec![String::from("MATH1081"), String::from("COMP1927")], &None, &api).unwrap());
    }
